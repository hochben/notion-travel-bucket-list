...
![The map, embedded at the top of the Notion page. Green is visited, blue is being planned, sand is on the wishlist.](map.png)
...
a few seconds later. The list is an ordinary Notion database — one card per trip:

![The Destinations database in Notion. Each card is one trip, with its status, budget and travel month.](bucket-list.png)

This README explains how that works, first in plain language and then in detail.
...
| `map.png`, `bucket-list.png` | The two screenshots above. |
| `README.md` | This file. |
...
notice means the outlines were blocked — normally a network restriction, not a problem with
